...
function ProductCard({ product }: { product: ReturnType<typeof selectFilteredProducts>[0] }) {
  ...
  // ── Zustand favorites hook ─────────────────────────────────────────────────
  // useFavorites() truy cập Zustand store trực tiếp — không cần selector Redux
  ...
  return (
    <article className="product-card">
      <div className="product-card__image-wrap">
        ...
        {/* ── Heart button (Zustand) ── */}
        <button
          id={`fav-btn-${product.id}`}
          className={`fav-btn ${isFav ? 'fav-btn--active' : ''}`}
          onClick={() => toggleFavorite(product.id)}
          aria-label={isFav ? `Bỏ yêu thích ${product.name}` : `Yêu thích ${product.name}`}
...
export function ProductList() {
  ...
  // ── RTK Query approach — chạy song song, dùng data để seed store ──────────
